...
class LayoutViewServerEndpoint(WebSocketEndpoint):
    ...
    def _build_layer_dict(
        self,
        layer: lay.LayerPropertiesNode,
        iter: lay.LayerPropertiesIterator,
        children: list[dict[str, object]] | None = None,
    ) -> dict[str, object]:
        """Build a dictionary representation of a layer."""
        layer_dict: dict[str, object] = {
            ...
            "img": base64.b64encode(
                self.layout_view.icon_for_layer(iter, 50, 25, 1).to_png_data()
            ).decode("ASCII"),
        }
        
        if children is not None:
            layer_dict["children"] = children
            layer_dict["empty"] = all(c["empty"] for c in children)
        else:
            layer_dict["empty"] = (
                self.current_cell().bbox_per_layer(layer.layer_index()).empty()
            )
    ...
    def layer_dump(
        ...
        """Dump layer properties to a list of dictionaries."""
        if iter is None:
        ...
        def should_continue() -> bool:
            if iter.at_end():
                return False
            if end_iter is not None and iter == end_iter:
                return False
            return True
        ...
        while should_continue():
            ...
            if layer.has_children():
                ...
                js.append(self._build_layer_dict(layer, iter, children))
            else:
                js.append(self._build_layer_dict(layer, iter))
    ...
    async def timer(self, websocket: WebSocket) -> None:
        def update() -> None:
            try:
                ...
                print(f"Error in image_updated: {e}")
        ...
        try:
            while True:
                try:
                    ...
                    print(f"Error in layout_view.timer: {e}")
            ...
            try:
                await websocket.send_json({"msg": "error", "details": f"Rendering error: {e}"})
            except Exception:
                pass
